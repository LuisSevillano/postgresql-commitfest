				...
				const char *agginitval)
{
	...
	/*
	 * Remember if trans function is strict as we need to make sure the
	 * inverse trans function has the same strict setting
	 */
	...
	if (agginvtransfnName)
	{
		 ...
		 * the transition function, so we can just borrow the argument details
		 * from the transition function and try to find a function with a
		 * matching signature.
		 */
