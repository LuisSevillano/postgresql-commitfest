...
int2_sum_inv(PG_FUNCTION_ARGS)
{
	...
	if (PG_ARGISNULL(0))
	{
		...
		/*
		 * This is the first non-null input, so we'll return
		 * 0 - input. When called as an aggregate function
		 * this should not happen, but we should probably
		 * get this right anyway.
		 */
		newval = (int64) 0 - PG_GETARG_INT16(1);
		PG_RETURN_INT64(newval);
	}
...
int4_sum_inv(PG_FUNCTION_ARGS)
{
	...
	if (PG_ARGISNULL(0))
	{
		 ...
		 * get this right anyway.
		 */
		newval = (int64) 0 - PG_GETARG_INT32(1);
		PG_RETURN_INT64(newval);
	}
	/*
	 * If we're invoked as an aggregate, we can cheat and modify our first
